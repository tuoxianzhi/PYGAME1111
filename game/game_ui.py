...
class Game_UI():#血条UI
    ...
    def display(self,screen,player,sett):
        pygame.draw.rect(screen,self.background_color,self.health_background)
        if player.health > 0:
            health_rect = (10,10,self.health_background[2]*player.health/100,40)
            pygame.draw.rect(screen,player.color,health_rect)
        lv = pygame.font.SysFont('Comic Sans MS',24).render(f"lv {player.level}",False,(255,255,255))
