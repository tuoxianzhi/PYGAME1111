...
class Boat():  # 太空船——（玩家与敌军的原型）
    def __init__(self, lenth=100.0, cen_pos=(0.0, 0.0), shape=[0, 1, 2], theta=0.0, color=(291, 241, 165), v=(0, 0),
                 bullet_v=10, a=0.1, cooldown=5, health=100, bullet_number=1, bullet_scatter=pi / 3, bullet_type=0,
                 damage=5, delta_vk=0.1):
    ...
    def display(self, screen, to_pos, playerv):
        ...
        if (len(point_list) % 2 == 0):
            n = len(point_list) // 2
            fire_point_list.append(
                ((point_list[n - 1][0] + point_list[n][0]) / 2, ((point_list[n - 1][1] + point_list[n][1]) / 2)))
            ...
            fire_point_list.append(((point_list[n][0] + point_list[n + 1][0]) / 2 + sin(theta) * lenth,
                                    (point_list[n][1] + point_list[n + 1][1]) / 2 + cos(theta) * lenth))
    ...
    def attack(self):
        ...
        if self.bullet_number == 1:
            b_list.append(
                Bullet(v=(self.bullet_v * sin(self.theta) + self.v[0], self.bullet_v * cos(self.theta) + self.v[1]),
                       theta=self.theta, color=self.color, pos=(
                        self.cen_pos[0] + sin(self.theta) * self.lenth, self.cen_pos[1] + cos(self.theta) * self.lenth),
            ...
            match self.bullet_type:
                case 0:
                    ...
                    for i in range(self.bullet_number):
                        b_list.append(
                            Bullet(v=(self.bullet_v * sin(theta) + self.v[0], self.bullet_v * cos(theta) + self.v[1]),
                                   theta=theta, color=self.color, pos=(self.cen_pos[0] + sin(self.theta) * self.lenth,
                                                                       self.cen_pos[1] + cos(self.theta) * self.lenth),
                                   damage=self.bullet_damage))
                        theta += delta_theta
                        theta %= 2 * pi
                case 1:
                    ...
                    if self.bullet_number % 2 == 1:
                        # 奇数的情况，则中心点发出弹药
                        b_list.append(Bullet(v=(
                            self.bullet_v * sin(self.theta) + self.v[0], self.bullet_v * cos(self.theta) + self.v[1]),
                            theta=self.theta, color=self.color, pos=(
                                self.cen_pos[0] + sin(self.theta) * self.lenth,
                                self.cen_pos[1] + cos(self.theta) * self.lenth), damage=self.bullet_damage))
                    # 再分开处理两边的情况
                    ...
                    for i in range(num):
                        b_list.append(Bullet(v=(
                            self.bullet_v * sin(self.theta) + self.v[0], self.bullet_v * cos(self.theta) + self.v[1]),
                            theta=self.theta, color=self.color, pos=(
                                p0[0] * (num - i) / num + p1[0] * i / num, p0[1] * (num - i) / num + p1[1] * i / num),
                            damage=self.bullet_damage))
                    theta = 2 * pi / self.side_num
...
class Player(Boat):
    def __init__(self, lenth=100.0, cen_pos=(0, 0), shape=[0, 1, 2], theta=0, color=(249, 241, 165), v=(0, 0),
                 bullet_v=10, a=0.1, cooldown=5, health=100,full_health=100, bullet_number=1, bullet_scatter=pi / 3, bullet_type=0):
        super().__init__(lenth, cen_pos, shape, theta, color, v, bullet_v, a, cooldown, health, bullet_number,
                         bullet_scatter, bullet_type)
        self.level = 1
        self.xp=0
    ...
    def level_up(self, sett, index):
        self.level += 1
        # 增加最大血量上限
        self.full_health=int(self.full_health*1.2)
        # 升级时回满血量
